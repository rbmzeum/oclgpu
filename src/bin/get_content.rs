...
async fn main() -> Result<(), Error> {
    // URL страницы
    let url = "https://arxiv.org/html/2412.15287v1";
    ...
    // Извлечение текста статьи (все параграфы)
    let selector = Selector::parse("p").unwrap();
    let mut article_text = String::new();
    for element in document.select(&selector) {
        article_text.push_str(&element.text().collect::<String>());
        article_text.push(' ');
    }
    ...
    // Вывод результатов
    println!("Article Text: {}", article_text);
    println!("Image URLs: {:?}", image_urls);

    println!("Инициализация ModernBERT модели...");
    let model = ModernBertModel::new().unwrap();
    ...
    let (hidden_states, attention_mask) = model.text_to_vector(&test_texts).unwrap();
